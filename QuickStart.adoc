...
as s function of time. Say Data within the Group `gt3r`, and within it the time, longitude and
...
image::Reynolds_dmr.png[]


NOTE: The _xml_ is a interactive document that allows to click and collapse the information on the _DMR_. xref:Reynolds_dmr[Figure 3] has all the _nodes_ associated with _Attributes_ collapsed. 


From the sst.mnmean.nc.gz DMR in xref:Reynolds_dmr[Figure 3], you can see that the dataset consists of:


* Dataset declaration.
* Dimensions declaration by their name and size
* Numeric types of the _Array_ data with appropriate _Dimension_ and _Attribute_ declarations in their nodes (not visible on xref:Reynolds_dmr[Figure 3]).
* Global Attributes declaration.


The _DMR_ in xref:Reynolds_dmr[Figure 3] broadly allows another interactive inspection
of a remote dataset.
...
This URL on a browser results in the DMR shown in xref:Reynolds_dmr_sst[Figure 4].

[[Reynolds_dmr_sst]]
.DMR showing only the variable `sst` and the attributes of the dataset.
image::Reynolds_dmr_sst.png[]
...
referenced in xref:DAPRequestForm[Figure 2], the CE to discard all variables except `delta_time`, `lon_ph` and `lat_ph` from the responsed, yields the following (_XML_) _DMR_

[[Atlas03_dmr_constrained]]
.DMR showing 3 variables inside the nested Group hierarcy `/gt3r/heights/`.
