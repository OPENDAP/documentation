...
header. All of the data access clients that have been working with EDL
authentication have been doing so because the authentication endpoints support
the most fundamental type of web authentication, HTTP BASIC. Crucially, even the
...
The transaction
is conducted over a TLS (HTTPS) connection so the communication may be
considered secure.
